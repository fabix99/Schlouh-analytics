...
from dashboard.utils.projections import get_league_quality_score
...
def _apply_profile_to_df(df: pd.DataFrame, profile_key: str) -> pd.DataFrame:
    """Apply profile preset filters. profile_key is the label (e.g. 'Young goalscorer'). Returns filtered DataFrame."""
...
# ---------------------------------------------------------------------------
# Hero — one value proposition + primary CTA (match Scouts home standard)
# ---------------------------------------------------------------------------
st.markdown(
    """
    <div class="hero-v2">
        <div class="hero-v2-title">⚖️ Compare Players</div>
        <div class="hero-v2-sub">
            Deep comparison with cross-league adjustment and tactical fit analysis.
            Select 2–5 players to compare side-by-side.
        </div>
        <div class="hero-v2-tagline">Use Find players to discover candidates, then add them here.</div>
...
# Primary CTA: Find players (gold); secondary nav
_btn_col, _gap_col, _back_col = st.columns([2, 0.5, 1])
with _btn_col:
    if st.button("Find players", key="compare_hero_find_players", type="primary", use_container_width=True):
        st.switch_page("pages/8_🔎_Discover.py")
with _back_col:
    if st.button("← Back to Home", key="compare_hero_home", use_container_width=True):
...
# Visual anchor when empty or only 1: spotlight-style card
if len(st.session_state.compare_list) < MIN_COMPARE:
    need = MIN_COMPARE - len(st.session_state.compare_list)
    st.markdown(
        f"""
        <div class="spotlight-card">
            <div class="spotlight-badge">Add 2–5 players</div>
            <div class="spotlight-name">Compare side-by-side</div>
            <div class="spotlight-meta">
                Add {need} more player(s) to run the comparison. Use filters below, then search or browse to add. Use <strong>Find players</strong> to discover more.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
if df_candidates.empty:
    ...
    if idx_best.empty:
        df_candidates_one = pd.DataFrame()
    else:
...
if st.session_state.compare_list:
    ...
    for i, player_id in enumerate(st.session_state.compare_list):
        ...
        if player_rows.empty:
            st.warning(f"Player id {player_id} not found in data. Remove from compare list.")
            continue
        ...
        if player_info is None:
            ...
            if not default_scope_rows.empty:
                player_info = default_scope_rows.iloc[0]
            else:
                player_info = player_rows.sort_values("season", ascending=False).iloc[0]
        ...
        with selected_cols[i]:
            st.markdown(
                f'<div class="compare-queue-card"><div class="name">{player_info["player_name"]}</div></div>',
                unsafe_allow_html=True,
            )
...
# ---------------------------------------------------------------------------
# Comparison Display (when 2–5 players selected) — show first so users see results immediately
# ---------------------------------------------------------------------------
if len(st.session_state.compare_list) >= MIN_COMPARE:
    st.markdown("---")
    ...
    # Cross-league: placeholder for future league-equivalent feature
    unique_leagues = set(leagues)
    if len(unique_leagues) > 1:
        st.info(
            "📊 **Coming soon:** We're adding a league-equivalent calculator so you can calibrate stats by league "
            "difficulty and compare players across competitions on a level playing field."
        )
    ...
    for i, (col, player) in enumerate(zip(cols, players_data)):
        with col:
            ...
            league_quality = get_league_quality_score(
                player.get("league_name", "Unknown"),
                competition_slug=player.get("competition_slug"),
                season=player.get("season"),
            )
            strength_fmt = f"{league_quality:.2f}" if league_quality > 1.0 else f"{league_quality:.0%}"
            st.markdown(
                f'<div class="compare-player-card" style="border-color:{PLAYER_COLORS[i]};">'
                f'<div class="player-name">{player["player_name"]}</div>'
                f'<div class="player-meta">{pos} · {flag} {player.get("league_name", "")} {player.get("season", "")}</div>'
                f'<div class="player-extra" style="display:flex;justify-content:space-between;margin-bottom:8px;">'
                f'<span>League strength</span><span style="color:#C9A840;" title="1.0 = PL baseline (data-derived when available)">{strength_fmt}</span></div>'
                f'<div class="player-extra">Age: {format_metric(player.get("age_at_season_start"), decimals=0)} · Rating: {format_rating(player.get("avg_rating"))}</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
    ...
    # -----------------------------------------------------------------------
    # Combined Radar Comparison (visual anchor for this page)
    # -----------------------------------------------------------------------
    st.markdown("---")
    st.markdown("<div class='section-header'>Combined radar</div>", unsafe_allow_html=True)
    ...
    for i, player in enumerate(players_data):
        ...
        for stat_key, stat_label in RADAR_STATS_UNIVERSAL:
            if stat_key in player.index:
                raw_val = player[stat_key]
                if stat_key in pool.columns and not pool.empty:
                    pct = float(get_percentile_zscore(raw_val, pool[stat_key].dropna()))
                    if pd.isna(pct):
                        pct = 50.0
                else:
                    pct = 50.0
    ...
    for stat_key, stat_label in compare_stats:
        ...
        for i, player in enumerate(players_data):
            ...
            # Raw values only; league-equivalent calibration coming later
            display_val = f"{val:.2f}" if not pd.isna(val) else "N/A"
            row[f"Player {i+1}"] = display_val
            row[f"Player {i+1}_tooltip"] = ""
    ...
    def highlight_best(s: pd.Series) -> list:
        """Highlight the best value in each row (series of display strings)."""
        try:
    ...
    if len(available_teams) > 0:
        ...
        if selected_team:
            ...
            # Fit score expects team_data (stats) + team_tactical_profile; no position-averaged stats in this view, so use tactical profile for both
            ...
            for i, (col, player) in enumerate(zip(fit_cols, players_data)):
                with col:
                    ...
                    st.markdown(
                        f'<div class="fit-score-card" style="border-color:{color};">'
                        f'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">'
                        f'<span class="player-name">{player["player_name"]}</span>'
                        f'<span style="font-size:1.5rem;font-weight:700;color:{color};">{grade}</span></div>'
                        f'<div style="background:#21262D;border-radius:4px;height:8px;margin-bottom:8px;">'
                        f'<div style="background:{color};border-radius:4px;height:8px;width:{score}%"></div></div>'
                        f'<div class="explanation">{fit_result["explanation"]}</div></div>',
                        unsafe_allow_html=True,
                    )
...
# ---------------------------------------------------------------------------
# Add more players (optional) — below comparison so results are seen first
# ---------------------------------------------------------------------------
if len(st.session_state.compare_list) < MAX_COMPARE:
    ...
    st.markdown("---")
    st.markdown("<div class='section-header'>Add more players (optional)</div>", unsafe_allow_html=True)
    if need_more > 0:
        st.markdown(f"**Select at least {need_more} more player(s) to compare (min {MIN_COMPARE}, max {MAX_COMPARE}):**")
    else:
        st.markdown(f"**You can add up to {remaining} more player(s) to compare:**")
    
    def _normalize_search_text(text: Optional[str]) -> str:
        """Normalize string for accent-insensitive search (NFKD + ASCII)."""
    ...
    if n_candidates == 0:
        st.info("No candidates match your filters. Use **Find players** to discover or relax filters above.")
        ...
        if not matches.empty:
            ...
            for idx, player in matches.iterrows():
                ...
                with cols[0]:
                    ...
                    st.markdown(
                        f'<div class="compare-candidate-row"><span class="name">{player["player_name"]}</span>'
                        f'<span class="meta">{pos_label} · {player["team"]} · {player["league_name"]} {player["season"]}</span></div>',
                        unsafe_allow_html=True,
                    )
                with cols[1]:
            ...
            st.info("No players found in the filtered list. Try relaxing filters, a different name, or use **Find players** to discover and add players to compare.")
    elif search_clean:
    ...
    if not df_candidates_one.empty:
        ...
        for i in range(len(browse_df)):
            ...
            with cols[0]:
                st.markdown(
                    f'<div class="compare-candidate-row"><span class="name">{row["player_name"]}</span>'
                    f'<span class="meta">{pos_label} · {row["team"]} · {flag} {row["league_name"]} {row["season"]} · Rating {row.get("avg_rating", 0):.2f}</span></div>',
                    unsafe_allow_html=True,
                )
            with cols[1]:
...
if len(st.session_state.compare_list) > 0 and len(st.session_state.compare_list) < MIN_COMPARE:
    st.info(f"Add at least {MIN_COMPARE - len(st.session_state.compare_list)} more player(s) to see the comparison. Use **Find players** to discover candidates.")

# ---------------------------------------------------------------------------
# Coverage & attribution (match Scouts home)
# ---------------------------------------------------------------------------
st.markdown("---")
...
if df_all is not None and not df_all.empty:
    ...
    _c1.metric("Players", f"{df_all['player_id'].nunique():,}")
    _c2.metric("Competitions", df_all["competition_slug"].nunique())
    _c3.metric("Seasons", df_all["season"].nunique())
    _c4.metric("Appearances", f"{int(df_all['appearances'].sum()):,}")
st.markdown(
    '<p class="data-attribution">Data sourced from SofaScore. Default scope: current season, leagues + UEFA. Use <strong>Find players</strong> to change scope.</p>',
    unsafe_allow_html=True,
)
st.markdown("---")
if st.button("← Back to Find Players", key="compare_footer_discover", use_container_width=True):
    st.switch_page("pages/8_🔎_Discover.py")
