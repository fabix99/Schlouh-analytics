...
def _render_playing_style(
    ...
    stats_in_pool = [s for s in radar_stats if s in pool_for_pct.columns]
    if stats_in_pool:
        pool_for_pct = compute_percentiles_zscore(pool_for_pct, [], stats_in_pool)
    ...
    for s in radar_stats:
        if s in tac_row.index:
            if not team_in_pool.empty and f"{s}_pct" in team_in_pool.columns:
                ...
                val = tac_row[s]
                pct = float(get_percentile_zscore(val, pool_for_pct[s].dropna()))
                if pd.isna(pct):
                    pct = 50.0
            else:
                pct = 50.0
